...
namespace RedBeanPHP\Driver {
    ...
    class RPDO implements Driver
    {
        ...
        protected function bindParams($statement, $bindings)
        {
            foreach ($bindings as $key => &$value) {
                if (is_integer($key)) {
                    ...
                    } elseif (!$this->flagUseStringOnlyBinding && AQueryWriter::canBeTreatedAsInt($value) && abs(
                            $value
                        ) <= $this->max) {
        ...
        public function __construct($dsn, $user = null, $pass = null)
        {
            ...
            if ((strpos($this->dsn, 'sqlite') === 0 && PHP_MAJOR_VERSION === 5 && PHP_MINOR_VERSION === 3) || defined(
                    'HHVM_VERSION'
    ...
    class OODBBean implements\IteratorAggregate,\ArrayAccess,\Countable,Jsonable
{
     ...
     */
    protected
    static $errorHandlingFUSE = false;
     ...
     */
    public
    static function setAutoResolve($automatic = true)
    {
     ...
     */
    public
    static function setMetaAll($beans, $property, $value)
    {
     ...
     */
    private
    function getSharedList($type, $redbean, $toolbox)
    {
     ...
     */
    private
    function getOwnList($type, $redbean)
    {
        $type = $this->beau($type);
        ...
        if ($this->getID()) {
            ...
            if (!is_numeric($firstKey) || $firstKey === null) {
                ...
                $beans = $redbean->find(
                    $type,
                    array(),
                    " {$joinSql} $myFieldLink = :slot0 " . $this->withSql,
                    $bindings
                );
            } else {
     ...
     */
    public
    function initializeForDispense($type, BeanHelper $beanhelper)
    {
     ...
     */
    public
    function setBeanHelper(BeanHelper $helper)
    {
     ...
     */
    public
    function import($array, $selection = false, $notrim = false)
    {
     ...
     */
    public
    function importFrom(OODBBean $sourceBean)
    {
        $this->__info['tainted'] = true;
        $this->__info['changed'] = true;
     ...
     */
    public
    function inject(OODBBean $otherBean)
    {
     ...
     */
    public
    function export($meta = false, $parents = false, $onlyMe = false, $filters = array())
    {
        $arr = array();
     ...
     */
    public
    function with($sql, $bindings = array())
    {
        $this->withSql = $sql;
     ...
     */
    public
    function withCondition($sql, $bindings = array())
    {
     ...
     */
    public
    function isListInExclusiveMode($listName)
    {
            ...
            if (isset($this->__info["sys.parentcache.$property"])) {
                ...
                if (!is_null($this->properties[$fieldLink])) {
                    ...
                    if ($bean->id !== $this->properties[$fieldLink] && self::$autoResolve) {
                        $type = $this->beanHelper->getToolbox()->getWriter()->inferFetchType(
                            $this->__info['type'],
                            $property
                        );
                        if (!is_null($type)) {
    ...
    function __set($property, $value)
    {
        ...
        $exists = isset($this->properties[$property]);
        $fieldLink = $property . '_id';
        $isFieldLink = (($pos = strrpos($property, '_id')) !== false) && array_key_exists(
     ...
     */
    public
    function setProperty($property, $value, $updateShadow = false, $taint = false)
    {
        $this->properties[$property] = $value;
     ...
     */
    public
    function getMeta($path, $default = null)
    {
     ...
     */
    public
    function moveMeta($path, $value = null)
    {
     ...
     */
    public
    function copyMetaFrom(OODBBean $bean)
    {
    ...
    function __call($method, $args)
    {
        ...
        if (!method_exists($this->__info['model'], $method)) {
            ...
            if (in_array(
                $method,
                array('update', 'open', 'delete', 'after_delete', 'after_update', 'dispense'),
                true
            )) {
                return null;
            }
     ...
     */
    public
    function traverse($property, $function, $maxDepth = null)
    {
        $this->via = null;
     ...
     */
    public
    function hasListChanged($property)
    {
     ...
     */
    public
    function link($typeOrBean, $qualification = array())
    {
    ...
    function countOwn($type)
    {
        ...
        if ($this->getID()) {
            ...
            if (!is_numeric($firstKey) || $firstKey === null) {
                $bindings = $this->withParams;
                $bindings[':slot0'] = $this->getID();
                $count = $this->beanHelper->getToolbox()->getWriter()->queryRecordCount(
                    $type,
                    array(),
                    " {$joinSql} $myFieldLink = :slot0 " . $this->withSql,
                    $bindings
                ...
                $count = $this->beanHelper->getToolbox()->getWriter()->queryRecordCount(
                    $type,
                    array(),
                    " {$joinSql} $myFieldLink = ? " . $this->withSql,
                    $bindings
                );
            }
        }
     ...
     */
    public
    function &aggr($list, $property, $type = null)
    {
        $this->via = null;
...
namespace RedBeanPHP\QueryWriter {
    ...
    abstract class AQueryWriter
    {
        ...
        public function queryRecordRelated($sourceType, $destType, $linkIDs, $addSql = '', $bindings = array())
        {
            ...
            list($sourceTable, $destTable, $linkTable, $sourceCol, $destCol) = $this->getRelationalTablesAndColumns(
                $sourceType,
                $destType
            );
	 ...
	 */
            ...
            foreach ($bean as $property => $value) {
                ...
                if ($value instanceof OODBBean) {
                    ...
                    if (strpos($property, 'own') === 0) {
                        list($ownAdditions, $ownTrashcan, $ownresidue) = $this->processGroups(
                            $originals,
                            $value,
                            $ownAdditions,
                            $ownTrashcan,
                        ...
                        list($sharedAdditions, $sharedTrashcan, $sharedresidue) = $this->processGroups(
                            $originals,
                            $value,
                            $sharedAdditions,
                            $sharedTrashcan,
        ...
        public function wipe($type)
        {
            try {
                ...
                if (!$this->writer->sqlStateIn(
                    $exception->getSQLState(),
                    array(QueryWriter::C_SQLSTATE_NO_SUCH_TABLE)
                )) {
                    throw $exception;
                }
            ...
            if (!$this->oodb->isChilled($bean->getMeta('type'))) {
                ...
                if ($doFKStuff) {
                    if (strrpos($columnNoQ, '_id') === (strlen($columnNoQ) - 3)) {
                        ...
                        if (is_array($isLink) && !count(
                                array_diff($isLink, array_keys($this->writer->getColumns($table)))
                            )) {
...
namespace RedBeanPHP {
    ...
    class AssociationManager extends Observable
    {
        ...
        {
            if ($this->oodb->isFrozen() || !$this->writer->sqlStateIn(
                    $exception->getSQLState(),
                    array(
                        QueryWriter::C_SQLSTATE_NO_SUCH_TABLE,
                        QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN
